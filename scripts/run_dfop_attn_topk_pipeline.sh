...
run_fusion_task() {
  ...
  seed_stage1_from_top1 "$top" "$task"
...
seed_stage1_from_top1() {
  # Stage-1 layer costs are independent of top-k; reuse attn top1 caches for top2+.
  local top="$1" task="$2"
  if [[ "$top" == "1" ]]; then
    return 0
  fi
  local src="$FUSION_ROOT/cache/${task}_${MODE}_r${RANK}_top1/stage1"
  local dst="$FUSION_ROOT/cache/${task}_${MODE}_r${RANK}_top${top}/stage1"
  if [[ ! -d "$src" ]]; then
    log "$top" "[cache] no top1 stage1 for $task; will compute fresh"
    return 0
  fi
  mkdir -p "$dst"
  local have=0
  have=$(find "$dst" -maxdepth 1 -name 'layer_cost_*.pt' 2>/dev/null | wc -l | tr -d ' ')
  local need=0
  need=$(find "$src" -maxdepth 1 -name 'layer_cost_*.pt' 2>/dev/null | wc -l | tr -d ' ')
  if [[ "$have" -ge "$need" && "$need" -gt 0 && -f "$dst/stage1_manifest.json" ]]; then
    log "$top" "[cache] stage1 already populated for $task ($have files)"
    return 0
  fi
  log "$top" "[cache] copy stage1 $task top1 -> top${top} ($need layer_cost files)"
  cp -a "$src"/. "$dst"/
}

reuse_target_eval_from_top1() {
  local top="$1" task="$2" eval_root="$3"
  if [[ "$top" == "1" ]]; then
    return 0
  fi
  local src="$STAGE_ROOT/top1/pre_sft/$task/target/primary"
  local dst="$eval_root/$task/target/primary"
  if [[ -d "$dst" ]] && { has_lm_eval_results "$dst" || has_malay_metrics "$dst"; }; then
    return 0
  fi
  if [[ -d "$src" ]] && { has_lm_eval_results "$src" || has_malay_metrics "$src"; }; then
    mkdir -p "$(dirname "$dst")"
    rm -rf "$dst"
    cp -a "$src" "$dst"
    log "$top" "[cache] reuse target eval $task from top1"
  fi
}
...
run_eval_job() {
  ...
  if [[ "$variant" == "target" ]]; then
    reuse_target_eval_from_top1 "$top" "$task" "$eval_root"
  fi
  if [[ "$task" == "malay" ]]; then
...
write_summary() {
  ...
  local pipeline_log="$STAGE_ROOT/top${top}/pipeline.log"
  conda activate lm-eval
  # NOTE: use shell locals for tee paths; only pass PRE/POST/TOP into python env.
  TOP="$top" PRE="$pre" POST="$post" python - <<'PY' | tee "$summary" | tee -a "$pipeline_log"
import csv, json, os
