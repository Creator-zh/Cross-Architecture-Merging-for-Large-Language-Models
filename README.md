...
2. **DF-OT-Procrustes（本仓库新增方法，简称 DFOP）**：全流程不使用数据、tokenizer 或 forward；对 Q/K/V/O/Gate/Up/Down 七类矩阵分别计算跨模型层代价与异构维度映射，支持行 softmax top-k、严格平衡 OT，以及 QK/VO/FFN 共享路由。实现位于 `core/dfop/`，入口是 `scripts/run_dfop_fusion.py` 和 `scripts/run_dfop_tasks.py`。
